...
# Configurable constraint — change here to propagate everywhere.
TODO_TITLE_MAX_LENGTH: int = 500


class TodoStatus(str, Enum):
    """Lifecycle state of a Todo: active or completed."""
...
class FilterCriteria(str, Enum):
    """View selection for listing Todos. Used by Application Layer only."""
...
class TodoId:
    """Stable, globally-unique identifier for a Todo. UUID v4 string. Immutable."""
    ...
    def generate(cls) -> TodoId:
        """Generate a fresh UUID v4 TodoId."""
...
@dataclass(frozen=True)
class TodoTitle:
    """Human-readable description of work. Non-empty string, max 500 chars, trimmed.

    Raises:
        InvalidTitleError: if blank or exceeds TODO_TITLE_MAX_LENGTH characters.
    """

    value: str

    def __post_init__(self) -> None:
        # frozen=True prevents normal attribute writes; use object.__setattr__ to trim.
