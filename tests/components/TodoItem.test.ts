...
describe('TodoItem — display mode', () => {
  ...
  it('has an accessible label associated with the checkbox', async () => {
    const wrapper = await mountSuspended(TodoItem, {
      props: { todo: ACTIVE_TODO, isEditing: false },
    })
    const label = wrapper.find(`label[for="todo-checkbox-${ACTIVE_TODO.id}"]`)
    expect(label.exists()).toBe(true)
  })

  it('applies line-through style when todo is completed', async () => {
    const wrapper = await mountSuspended(TodoItem, {
      props: { todo: COMPLETED_TODO, isEditing: false },
    })
    const titleEl = wrapper.get('[data-testid="todo-title"]')
    expect(titleEl.classes()).toContain('line-through')
  })
})
